...
var _selected_shapes: Array[FunctionBoxCollisionShape2D] = [];
var linked_provider: FrameDataProviderTool;

var push_count: int = 0;

signal shape_input(event: InputEvent, shape: FunctionBoxCollisionShape2D, layer: EditableFunctionBoxLayer);
...
func clear_selected() -> void:
	for shape: FunctionBoxCollisionShape2D in self._selected_shapes:
...
func _shape_selected(shape: FunctionBoxCollisionShape2D) -> void:
	shape.debug_color = self.default_debug_color.lerp(selected_color,selected_blend_fac);

func _shape_unselected(shape: FunctionBoxCollisionShape2D) -> void:
...
func _input_event(viewport: Viewport, event: InputEvent, shape_idx: int) -> void:
	var shape: FunctionBoxCollisionShape2D = self.shape_owner_get_owner(self.shape_find_owner(shape_idx));
	self.shape_input.emit(event, shape, self);

func is_shape_selected(shape: FunctionBoxCollisionShape2D) -> bool:
